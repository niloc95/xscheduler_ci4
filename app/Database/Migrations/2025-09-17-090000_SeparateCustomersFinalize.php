...
FROM `{$usersTable}` u
LEFT JOIN `{$customersTable}` c_e ON (u.email IS NOT NULL AND c_e.email = u.email)
LEFT JOIN `{$customersTable}` c_p ON (u.phone IS NOT NULL AND c_p.phone = u.phone)
WHERE u.role = 'customer'
  AND c_e.id IS NULL
  AND c_p.id IS NULL;
SQL;
        $db->query($sqlInsertCustomers);
...
JOIN `{$usersTable}` u ON u.id = a.user_id AND u.role = 'customer'
LEFT JOIN `{$customersTable}` c ON (
...
SQL;
        $db->query($sqlUpdateApptCustomer);
