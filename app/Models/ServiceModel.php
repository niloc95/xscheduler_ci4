            ...
            SELECT 
                ...
                ROUND(
                    ((COUNT(CASE WHEN a.start_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 30 DAY) THEN 1 END) - 
                      COUNT(CASE WHEN a.start_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 60 DAY) AND a.start_at < DATE_SUB(UTC_TIMESTAMP(), INTERVAL 30 DAY) THEN 1 END)) /
                     NULLIF(COUNT(CASE WHEN a.start_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 60 DAY) AND a.start_at < DATE_SUB(UTC_TIMESTAMP(), INTERVAL 30 DAY) THEN 1 END), 0)) * 100,
                    1
                ) as growth
            FROM xs_services s
