...
- Use **PHPUnit**.
- Integration tests MUST run against test DB config only. Locally, run the DB suite with `vendor/bin/phpunit -c phpunit.mysql.xml.dist` (the default `phpunit.xml.dist` has no DB connection, so any service that touches `$model->db` at construction — e.g. `BookingMetricsService` — errors).
...
- **`.env.example` MUST end with a trailing newline.** CI builds the test/setup env with `cp .env.example .env` then `echo "KEY = val" >> .env`; without the trailing newline the first appended key concatenates onto the last line (`...false CI_ENVIRONMENT = testing`), and CI4's `DotEnv` rejects unquoted values containing spaces — `php spark` fatals and the PHPUnit + Setup-Wizard jobs never run. The two `ci-cd.yml` env steps now `printf '\n' >> .env` defensively, but keep the trailing newline in `.env.example`.
...
- `DashboardBoundaryServicesTest::testDashboardPageServiceBuildLandingViewData{IncludesAppointmentScope,PassesArrayProviderScope}` error at construction: they build `DashboardPageService` with a **mocked** `AppointmentModel` (whose `->db` is `null`), and `BookingMetricsService::__construct()` assigns `$this->appointments->db` to a non-nullable `BaseConnection $db` → `TypeError`. Fails before any tested logic runs. **Fix:** either inject a `BookingMetricsService` mock into the `DashboardPageService` constructor in those tests, or make `BookingMetricsService` resolve `db_connect()` when `$model->db` is null. Pre-existing; non-gating (the PHPUnit job is `continue-on-error`).
...
9. Fully green the `PHPUnit Tests` CI job (currently `continue-on-error`, non-gating; item 62 fixed its bootstrap fatals). Remaining, in order: (a) add `database.default.*` to the phpunit "Create test database config" step (migrate/seed use the `default` group, not `tests`); (b) repair the `2025-10-22-174832` migration-sequence gap so `migrate:refresh` completes; (c) fix the `DashboardBoundaryServicesTest` null-`db` mock construction errors; (d) restore coverage ≥40% or adjust the threshold. Only once all pass should `continue-on-error` be reconsidered.
...
| 62 | CI/CD Pipeline — chronic red fixed (npm audit + `.env` parse fatal) | done | The `WebScheduler CI/CD Pipeline` (`ci-cd.yml`) had been failing on every commit for many releases. Two independent root causes: **(a) Gating** — the `code-quality` job (the only job without `continue-on-error`) ran `npm audit --audit-level=moderate` and failed on 4 build-tool advisories (`vitest` critical, `vite`/`undici`/`brace-expansion` high). Fixed with `npm audit fix` (semver-compatible patches: `vitest 3.2.4→3.2.7`, `vite 6.4.2→6.4.3`, `undici 7.24.6→7.28.0`, `brace-expansion 2.1.1→2.1.2`); build output byte-identical, 0 vulnerabilities. **(b) Non-gating but masking all test signal** — `.env.example` had no trailing newline, so CI's `cp .env.example .env` + `echo "KEY = val" >> .env` concatenated the first appended key onto `features.apiAccess = false`, producing an unquoted value with spaces that CI4 `DotEnv` (`system/Config/DotEnv.php:179`) rejects → `php spark migrate:refresh`/`serve` fataled with `Class "CodeIgniter\Exceptions\InvalidArgumentException" not found`, so the PHPUnit and Setup-Wizard jobs never actually ran. Fixed by adding the trailing newline to `.env.example` and a defensive `printf '\n' >> .env` in both `ci-cd.yml` env-setup steps. Reproduced/verified locally with a standalone `DotEnv` parse harness. **(c)** With the `.env` fatal gone, the PHPUnit job progressed to `Fresh migrate test schema`, which then fataled with `ConfigException: Config\App::$baseURL "/" is not a valid URL` — the phpunit env step (unlike setup-test) never set `app.baseURL` and `.env.example` ships it empty. Added `app.baseURL = 'http://localhost:8080/'` to that step. **(d)** `php spark migrate:refresh -n App --all` then hit an interactive `"Are you sure you want to refresh? [y, n]:"` prompt — CI has no TTY, so `CLI::prompt()`/`InputOutput::input()` reads `false` and throws a `TypeError`; added the `-f` (force) flag. **Result:** overall pipeline is **green** (verified run 29809595982 — Build/Code-Quality/Setup-Wizard/Performance all pass; `code-quality`, the only gating job, is green). The **PHPUnit job stays red** (`continue-on-error`, non-gating). Peeling back its fatals showed it had **never actually run** — a stack of latent misconfigs, each masked by the previous fatal. After (a)-(d), the next revealed blocker: **(e)** `php spark migrate:refresh` / `db:seed` use CI4's **`default`** DB group, but the phpunit env step only sets `database.tests.*` → `DatabaseException: Unable to connect to the database. Main connection [MySQLi]: Uninitialized string offset 0`. **To fully green the PHPUnit job** (a dedicated follow-up, tracked in Next Hardening Queue): (1) add `database.default.*` (hostname/database/username/password/DBDriver/port → the MySQL service) to the phpunit "Create test database config" step so migrate/seed connect; then it will surface the genuine test debt — (2) the `2025-10-22-174832` migration-sequence gap (may fail `migrate:refresh` itself), (3) the `<40%` coverage-threshold step, and (4) the `DashboardBoundaryServicesTest` null-`db` mock errors. None of these gate the pipeline or regressed from this work. |
